...
gff<-as.list(class(d1))
gff<-as.character(gff[[1]])
print(paste("The class of the eval data frame is: ",gff))

print(paste("The dimensions of the eval data frame are: ", dim(d1)[1],"rows by",dim(d1)[2]," columns"))
...
median(d1$score)
summary(d1$score)
d2<-filter(d1,score<3)
print(paste("The number of students giving a professor a score below 3 is: ",nrow(d2)))
hist(d1$score)
#Left skewness suggests MORE LIKELY to rate highly
...
```{r two-vars-eda}
sapply(d1,class)
boxplot(d1$bty_avg~d1$bty_f1lower)
...
#Same but for females only
ggplot(data = evals, aes(x = bty_f2upper, y = score)) +
  geom_jitter() +
  geom_smooth(method = "lm")
...
ggplot(data = fit, aes(y=fit$residuals,x=fit$fitted.values)) + geom_point() +
...
```{r bty-rel}
ggplot(data = d1, aes(x = bty_f1lower, y = bty_avg)) +
  geom_jitter()

#ggplot(data = d1, aes(x = bty_f1lower, y = bty_avg)) +  geom_point()

  summarise(d1,cor(bty_avg, bty_f1lower))
```
...
gfh
ggpairs(gfh)
gfh5<-rename(gfh,a=bty_f1lower,b=bty_m1lower,c=bty_f1upper,d=bty_f2upper,e=bty_m2upper,f=bty_m1upper,g=bty_avg)
ggpairs(gfh5,columns = 1:2)
...
```{r diag-mlr-model}
# type your code for the Exercise here, and Knit
ggplot(fit2,aes(y=fit2$residuals,x=fit2$fitted.values)) + geom_point() + geom_hline(yintercept = 0)
...
```{r summary-mlr-model}
# type your code for the Exercise here, and Knit
fit2<-lm(score~bty_avg + gender,d1)
summary(fit2)
...
```{r new-mlr-model}
# type your code for the Exercise here, and Knit
m_bty_rank<-lm(score~bty_avg + rank,d1)
summary(m_bty_rank)
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
fit3 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
...
```{r adj-rsq-select}
# type your code for the Exercise here, and Knit
ft1 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
f1<-summary(ft1)
ar2full<-f1$adj.r.squared
ar2full


ft1 <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
f1<-summary(ft1)
ar2rank<-f1$adj.r.squared
ar2rank


ft1 <- lm(score ~ rank + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
f1<-summary(ft1)
ar2eth<-f1$adj.r.squared
ar2eth

ft1 <- lm(score ~ rank + gender  + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
f1<-summary(ft1)
ar2lang<-f1$adj.r.squared
ar2lang

ft1 <- lm(score ~ rank + gender + language + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
f1<-summary(ft1)
ar2age<-f1$adj.r.squared
ar2age

ft1 <- lm(score ~ rank + gender + language + age  
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
f1<-summary(ft1)
ar2cls_perc_eval<-f1$adj.r.squared
ar2cls_perc_eval
...
ft1 <- lm(score ~ rank + gender + language + cls_perc_eval 
             ...
             + pic_outfit + pic_color, data = evals)
f1<-summary(ft1)
ar2cls_students<-f1$adj.r.squared
ar2cls_students
...
ft1 <- lm(score ~ rank + gender + language + cls_perc_eval 
             ...
             + pic_outfit + pic_color, data = evals)
f1<-summary(ft1)
ar2cls_credits<-f1$adj.r.squared
ar2cls_credits
...
f1<-summary(ft1)
ar2pic_outfit<-f1$adj.r.squared
ar2pic_outfit
...
ft1 <- lm(score ~ rank + gender + language + cls_perc_eval 
              ...
              + pic_color, data = evals)
f1<-summary(ft1)
bestmodel<-f1$adj.r.squared
bestmodel


ft1 <- lm(score ~ gender + language + cls_perc_eval 
              + cls_level  + cls_credits + bty_avg 
              + pic_color, data = evals)
f1<-summary(ft1)
bestmodelrank<-f1$adj.r.squared
bestmodelrank

ft1 <- lm(score ~ rank + language + cls_perc_eval 
              + cls_level  + cls_credits + bty_avg 
              + pic_color, data = evals)
f1<-summary(ft1)
bestmodelgender<-f1$adj.r.squared
bestmodelgender
...
ft1 <- lm(score ~ rank + language + cls_perc_eval 
          + cls_credits + bty_avg 
              + pic_color, data = evals)
f1<-summary(ft1)
bestmodelbty_avg<-f1$adj.r.squared
bestmodelbty_avg
