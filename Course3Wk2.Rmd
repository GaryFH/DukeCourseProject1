...
#sample things to do
#by(mlb11$runs, mlb11$team, summary)- no meaning since each team has only one observation
plot(y=mlb11$runs,x=mlb11$bat_avg)
hist(mlb11$runs)
summary(mlb11$runs)
sapply(mlb11[,2:12],summary)
d1<-mlb11
...
```{r runs-vs-at_bats}
plot(x = d1$at_bats, y = d1$runs)
#plot(x=at_bats,y=runs,data=d1) doesn't work?
...
m2<-lm(homeruns~runs,d1)
summary(m2)
m3<-lm(runs~homeruns,d1)
summary(m3)
plot(x=d1$homeruns,y=d1$runs)
...
```{r reg-with-line}
gfh<-ggplot(data = mlb11, aes(x = at_bats, y = runs)) +
  geom_point() +
...
```{r eval=FALSE}
x1<-mlb11 %>%  filter(at_bats == 5579) %>%  select(runs)
x1
```
...
estval = -2789.2429 + 0.6305 *5579
actval<-mlb11 %>%  filter(at_bats == 5579) %>%  select(runs)
res<-actval-estval
res
```
...
```{r runs-vs-sth-else}
m4<-lm(runs~strikeouts,d1)
summary(m4)

ggplot(data = d1, aes(x = strikeouts, y = runs)) +
  geom_point() +
  stat_smooth(method = "lm", se = T)
...
```{r pick-model}
m5<-lm(runs~strikeouts+at_bats,d1)
summary(m5)
...
m6<-lm(runs~strikeouts+at_bats+hits+homeruns+bat_avg+stolen_bases+wins,d1)
summary(m6)

summary(atbats<-lm(runs~at_bats,d1))
summary(wins<-lm(runs~wins,d1))
summary(batavg<-lm(runs~bat_avg,d1))
summary(hits<-lm(runs~hits,d1))
...
```{r runs-vs-new-vars}
summary(newobs<-lm(runs~new_obs,d1))
summary(newslug<-lm(runs~new_slug,d1))
summary(newonbase<-lm(runs~new_onbase,d1))
