...
hist(samp1$area)
summarise(samp1,statmean = mean(area), pop_med = median(area), 
            statvar = sd(area), pop_iqr = IQR(area),
            pop_min = min(area), pop_max = max(area),
            pop_q1 = quantile(area, 0.25),  # first quartile, 25th percentile
            pop_q3 = quantile(area, 0.75))  # third quartile, 75th percentile
...
aa<-mean(sapply(1:25,function(x) summarise(sample_n(ames,10),x_bar=mean(area)))$x_bar)

bb<-function(b){
samp<-sample_n(b,size = 10)
summarise(samp,x_bar = mean(area))
}

aa<-sapply(1:25,function(x)bb(ames))
#aa
mean(aa$x_bar)

bbb<-function(c){
samp2<-sample_n(c,size = 50)
summarise(samp2,x_bar = mean(area))
}
bb<-sapply(1:25,function(x)bbb(ames))
mean(bb$x_bar)
