...
mean(sample_n(ames,60)$area)
 for (i in 1:10){
print(mean(sample_n(ames,60)$area))
 }
...
summarise(samp,lower=mean(area)-qnorm(.975)*(sd(area)/sqrt(n)))
summarise(samp,upper=mean(area)+qnorm(.975)*(sd(area)/sqrt(n)))
```
...
rep_sample_n(size=60,reps=1000,replace=T) %>%
  
summarise(lower=mean(area)-qnorm(.975)*(sd(area)/sqrt(n)),upper=mean(area)+qnorm(.975)*(sd(area)/sqrt(n)))
...
ci2<- ci %>%
  mutate(capture_mu=ifelse(lower<mean(ames$area)&upper>mean(ames$area),1,0))

sum(ci2$capture_mu)/1000
...
rep_sample_n(size=60,reps=100000,replace=T) %>%
  
summarise(lower=mean(area)-qnorm(.995)*(sd(area)/sqrt(n)),upper=mean(area)+qnorm(.995)*(sd(area)/sqrt(n)))
