...
#---
##Example of good work on Rpubs
###https://rpubs.com/rodsveiga/252171
...
# We can compare the two groups (second group is factor or character) using the by() function together with the summary() function.
...
aa<-group_by(nc,habit)

aa1<-summarise(aa,mean_weight=mean(weight))
aa1

summarise(aa,mean(weight))
```
...
```{r }
#inference(y = nc$weight, x = nc$habit,  est = "mean", type = "ht",null = 0, method = "theoretical", alternative = "twosided")

inference(y = weight, x = habit, data = nc, statistic = "mean", type = "ht",null = 0, method = "theoretical", alternative = "twosided")
...
bb<-group_by(nc,mature)

summarise(bb,max(mage))

summarise(bb,min(mage))
...
###See if weight is influenced by mature


```{r pick-your-own}
inference(y=weight, x=mature, method = "theoretical",data = nc,alternative = "twosided",null = 0,statistic = "mean",type = "ht" )

t.test(weight ~ mature, data = nc, conf.level = 0.95)

inference(y=weight, x=mature, method = "theoretical",data = nc,alternative = "twosided",conf_level = .99,statistic = "mean",type = "ci" )
