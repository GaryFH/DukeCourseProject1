...
### The survey
####Github example https://github.com/guindilla/coursera-statistics-002/blob/master/unit5-lab/unit5-lab5.R


The press release for the poll, conducted by WIN-Gallup International, can be accessed [here]( https://github.com/guindilla/coursera-statistics-002/blob/master/unit5-lab/unit5-lab5.R).
...
atheist12<-filter(us12,response=="atheist")
Prop12<-nrow(atheist12)/nrow(us12)
Prop12
atheist05<-filter(us05,response=="atheist")
Prop05<-nrow(atheist05)/nrow(us05)
Prop05
```
...
Afg12<-filter(atheism ,nationality=="Afghanistan",year=="2012")
Arm12<-filter(atheism,nationality=="Armenia",year=="2012")
propAfgan<-nrow(filter(Afg12,response=="atheist"))/nrow(Afg12)
propAfgan
propArm<-nrow(filter(Arm12,response=="atheist"))/nrow(Arm12)
propArm
...
```{r me-plot}
d <- tbl_df(p <- seq(0, 1, 0.01))
n <- 1000

d1<-  mutate(d,me = 1.96*sqrt(p*(1 - p)/n))
ggplot(d1, aes(x = p, y = me)) +
  geom_line()
```
...
atheist12<-filter(spain12,response=="atheist")
Prop12<-nrow(atheist12)/nrow(spain12)
Prop12
atheist05<-filter(spain05,response=="atheist")
Prop05<-nrow(atheist05)/nrow(spain05)
Prop05


spain<-filter(atheism,nationality=="Spain")
...
```{r us-05-12}
us12 <-   filter(atheism,nationality == "United States" )
inference(y=us12$response, x=as.factor(us12$year),
